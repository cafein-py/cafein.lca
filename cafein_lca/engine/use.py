...
def electricity_factors(mix_shares):
    ...
    wtt = sum(s * f for s, f in
              zip(mix_shares, paths["wtt_energy_factor"])) - 1
...
def _hydrogen_factors(pathway, mix_shares):
    ...
    if pathway == _ELECTROLYSIS:
        energy = sum(s * f for s, f in
                     zip(mix_shares, h2["electrolysis_energy"]))
...
def run(params, mix_shares):
    ...
    if params.hydrogen_pathway:
        h2_wtt, h2_ghg = _hydrogen_factors(params.hydrogen_pathway,
                                           mix_shares)  # R29/R33
