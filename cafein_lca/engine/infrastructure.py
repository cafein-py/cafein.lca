...
def _imputable_share(infra_type, params, rail_allocation):
    ...
    if not infra_type:
        return 0.0
    lookup = float(conf.infrastructure_types[infra_type]
                   ["energy_imputable_share"] or 0)
    if rail_allocation:
        return lookup
...
def _per_vkm(materials, lifetime, annual_use, infra_type, params,
             rail_allocation):
    ...
    energy_per_km = (                                 # R42/R43
        ...
        + steel * (1 - rec)
        * conf.constant("infra_energy_virgin_steel_mj_per_kg")
        + steel * rec
        * conf.constant("infra_energy_recycled_steel_mj_per_kg")
    ) * 1000
    ghg_per_km = (                                    # R65/R66
        asphalt * conf.constant("infra_ghg_asphalt_g_per_kg")
        + cement * conf.constant("infra_ghg_cement_g_per_kg")
        + steel * (1 - rec)
        * conf.constant("infra_ghg_virgin_steel_g_per_kg")
        + steel * rec
        * conf.constant("infra_ghg_recycled_steel_g_per_kg")
    ) * 1000 / 1e3
    ...
    return (energy_alloc / (absolute_use * 1e6),      # R54/R55
            ghg_alloc * 1000 / (absolute_use * 1e6))  # R77/R78


def run(params, data):
    """Infrastructure energy [MJ/vkm] and GHG [g/vkm] (5_Infra R56/R79)."""
    e1, g1 = _per_vkm(data.infra1_materials, data.infra1_lifetime_years,
                      data.infra1_annual_use_mvkm, data.infra1_type, params,
                      data.infra_rail_allocation)
    e2, g2 = _per_vkm(data.infra2_materials, data.infra2_lifetime_years,
                      data.infra2_annual_use_mvkm, data.infra2_type, params,
                      data.infra_rail_allocation)
    share_1 = data.infra_share_1                      # R7; R8 = 1 - R7
    return (e1 * share_1 + e2 * (1 - share_1),        # R56
            g1 * share_1 + g2 * (1 - share_1))        # R79
