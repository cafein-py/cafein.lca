...
def battery_weight_kg(params):
    ...
    if not params.battery_capacity_kwh or not params.battery_chemistry:
        return 0.0
    spec = float(
        conf.battery_chemistries[params.battery_chemistry]["specific_energy_kwh_per_kg"]
    )
...
def _materials_burden(params, data, metric):
    ...
    for material, share in shares.items():
        if material in recycled:
            ...
            virgin = conf.material_intensity(f"virgin_{material}", region, metric)
            recyc = conf.material_intensity(f"recycled_{material}", region, metric)
            total += w * share * ((1 - rec) * virgin + rec * recyc)
        else:
            total += w * share * conf.material_intensity(material, region, metric)
    return total
...
def _battery_intensity(params, metric):
    ...
    suffix = (
        "coal_al"
        if params.production_region == "With Al smelting mostly form coal"
        else "default"
    )
    if metric == "energy":
        mfg = (
            float(row[f"mfg_energy_mmbtu_{suffix}"])
        ...
        mfg = float(row[f"mfg_ghg_g_{suffix}"]) / conf.constant("battery_reference_kwh")
